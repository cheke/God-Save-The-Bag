...
public class SVGParser {
	...
	private static class SVGHandler extends DefaultHandler {
		...
		private int opacityLevel = 0;
		private boolean boundsMode = false;

		private Float opacityf;
		...
		private void doColor(Properties atts, Integer color, boolean fillMode, Paint paint) {
			...
			paint.setColor(c);
			if(!opacity){
				opacityf = atts.getFloat("opacity");
			}
			if (opacityf == null) {
				opacityf = atts.getFloat(fillMode ? "fill-opacity" : "stroke-opacity");
			}
			if (opacityf == null) {
				paint.setAlpha(255);
			} else {
				paint.setAlpha((int) (255 * opacityf));
			}
		}
		...
		public void startElement(String namespaceURI, String localName, String qName, Attributes atts) {
			...
			if (localName.equals("svg")) {
				...
				if(getFloatAttr("opacity",atts)!=null&&getFloatAttr("opacity",atts)<1.0){
					opacity=true;
					opacityf=getFloatAttr("opacity",atts);
					opacityLevel=1;
				}
